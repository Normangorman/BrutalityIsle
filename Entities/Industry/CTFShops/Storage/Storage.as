...
void onInit(CSprite@ this)
{
	...
	// Purple Gold
	CSpriteLayer@ purple_gold = this.addSpriteLayer("mat_purple_gold", "MaterialPurpleGold.png", 16, 16);
	if (purple_gold !is null)
	{
		{
			purple_gold.addAnimation("default", 0, false);
			int[] frames = { 0, 1, 2 };
			purple_gold.animation.AddFrames(frames);
		}
		purple_gold.SetOffset(Vec2f(-7.0f, -10.0f));
		purple_gold.SetRelativeZ(1);
		purple_gold.SetVisible(false);
	}
...
void updateLayers(CBlob@ this, CBlob@ blob)
{
	...
	else if (blobName == "mat_purple_gold")
	{
		CSpriteLayer@ purple_gold = sprite.getSpriteLayer("mat_purple_gold");
		if (blobCount > 0)
		{
			if (blobCount >= 200)
			{
				purple_gold.SetFrameIndex(2);
			}
			else if (blobCount >= 100)
			{
				purple_gold.SetFrameIndex(1);
			}
			else
			{
				purple_gold.SetFrameIndex(0);
			}
			purple_gold.SetVisible(true);
		}
		else
		{
			purple_gold.SetVisible(false);
...
{
	return (blobName == "mat_stone" || blobName == "mat_wood" || blobName == "mat_gold" || blobName == "mat_purple_gold" || blobName == "mat_bombs" || blobName == "lantern");
}
